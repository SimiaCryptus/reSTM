...
class ClassificationTree[T](rootPtr: STMPtr[ClassificationTree.ClassificationTreeData[T]]) {
  ...
  class AtomicApi(priority: Duration = 0.seconds, maxRetries:Int = 1000)(implicit cluster: Restm, executionContext: ExecutionContext) extends AtomicApiBase(priority,maxRetries) {
    ...
    class SyncApi(duration: Duration) extends SyncApiBase(duration) {
      ...
      def init()(implicit classTag: ClassTag[T]) = sync { AtomicApi.this.init() }
    ...
    def init()(implicit classTag: ClassTag[T]) = atomic { ClassificationTree.this.init()(_,executionContext, classTag).map(_ => Unit) }
  ...
  class SyncApi(duration: Duration) extends SyncApiBase(duration) {
    ...
    def init()(implicit ctx: STMTxnCtx, executionContext: ExecutionContext, classTag: ClassTag[T]) = sync { ClassificationTree.this.init() }
  ...
  def init()(implicit ctx: STMTxnCtx, executionContext: ExecutionContext, classTag: ClassTag[T]) = {
    rootPtr.readOpt().map(prev => {
      prev.orElse(Option(ClassificationTree.newClassificationTreeData[T]()))
        .map(state => state->None).get
    }).flatMap(newRootData => rootPtr.write((newRootData._1)).map(_=>newRootData._2))
  }
